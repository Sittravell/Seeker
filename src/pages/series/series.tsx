...
import usePopularSeries from './use-popular-series';
...
export function Series() {
    ...
    return (
        <div className="min-h-screen bg-gray-900 text-white px-4 pb-4 pt-0 md:p-8 lg:p-8">
            <h1 className="text-3xl font-bold mb-8 text-purple-400">Popular Series</h1>

            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-6">
                {series.map((item, index) => {
                    const isRecommended = recommendedSet.has(`${item.mediaType}:${item.tmdbId}`);
                    const isLastElement = index === series.length - 1;
